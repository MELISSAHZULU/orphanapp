...
@Composable
fun DonationScreen(navController: NavController, donationViewModel: DonationViewModel) {
        ...
        Column(
            ...
            Button(
                onClick = { 
                    ...
                    // Create a new donation object (the ID will be generated by Firestore)
                    val newDonation = Donation(donorName = donorName, amount = amount, date = currentDate)
                    donationViewModel.addDonation(newDonation)
                    // Navigate to the history screen to see the new donation
                    navController.navigate("donation_history") 
                },
                modifier = Modifier.fillMaxWidth(),
                // Disable the button if the fields are empty
                enabled = donorName.isNotBlank() && amount.isNotBlank()
            ) {
                Text("Donate Now")
            }
        }
